...
select sub_category,avg(profit) from orders group by sub_category having avg(profit) > max(profit)/2
...
--13- write a query to print below 3 columns
...
--16-write a query to print below output from orders data. example output
...
with rnk as (
select *, rank() over(partition by dept_id order by salary desc,emp_age) as rn
from employee)
